...
public class AimUtil {
  ...
  private static Rotation2d shooterHeading = Rotation2d.kZero;
  ...
  /**
   * Set the target position that the AimUtil method uses for the periodic updateShooterConstants
   * calculation.
   *
   * @param targetPos x and y position of the target on the field
   * @param targetH final height that the ball should be at
   */
  public static void setTarget(Translation2d targetPos, double targetH) {
    // Aimutil already updates periodically,
    // so we don't want to waste cycles recalculating
    // if it's the same position
    if (targetPos.equals(targetPosition) && targetH == targetHeight) {
      return;
    }
  ...
  /**
   * Calculate the ball speed, robot heading, and shooter angle needed to shoot even while moving at
   * an arbitrary position on the field, and modify member variables to store these calculated
   * values
   */
  public static void updateShooterConstants() {
    ...
    AngularVelocity currentAngularVelocity =
        AngularVelocity.ofBaseUnits(driveState.Speeds.omegaRadiansPerSecond, RadiansPerSecond);

    if (targetPosition == null) {
      return;
    }

    ShooterMathResults results =
        runShooterMath(
            currentRobotSpeeds,
            currentRobotPose,
    ...
    shooterHeading = results.shooterHeading();
    ...
    Logger.recordOutput("AimUtil/outputRevps", rotationsPerSecond);
    Logger.recordOutput("AimUtil/exitAngle", exitAngle.in(Degrees));
    Logger.recordOutput("AimUtil/shooterHeading", shooterHeading);
   ...
   * @param shooterHeading Angle of robot
   * @param hangTime Time the ball spends in the air
   */
  public record ShooterMathResults(
      Angle exitAngle, LinearVelocity ballVelocity, Rotation2d shooterHeading, Time hangTime) {}

  /**
   * Given a chassis speeds, position, and angular velocity, calculate the optimal shooting
   * parameters to get a ball in the hub.
   *
   * @param currentRobotSpeeds driveState.Speeds
   * @param currentRobotPose driveState.Pose
   * @param currentAngularVelocity driveState.Speeds.omegaRadiansPerSecond (as an AngularVelocity
   *     object)
   ...
   * @param maxBallYPos The Y value for the highest point of the ball's curve, relative to the field
   * @return A ShooterMathResults record with the wanted exit angle, ball velocity, and robot
   *     heading
   */
  private static ShooterMathResults runShooterMath(
    ...
    double hangTime =
        ((Math.sqrt(maxBallYPos * 2) + Math.sqrt(2 * (maxBallYPos - targetHeight)))
    ...
    Translation2d futurePos =
        currentRobotPose
            ...
            .plus(
                new Translation2d(
                        currentRobotSpeeds.vxMetersPerSecond, currentRobotSpeeds.vyMetersPerSecond)
                    // causes feedback
                    .plus(
                        new Translation2d(
                            currentAngularVelocity.magnitude()
                                * Constants.Shooter.SHOOTER_DISTANCE_FROM_CENTER.in(Meters)
                                * Math.sin(currentRobotHeading),
                            currentAngularVelocity.magnitude()
                                * Constants.Shooter.SHOOTER_DISTANCE_FROM_CENTER.in(Meters)
                                * Math.cos(currentRobotHeading)))
    ...
    Translation2d shooterVelocityVec =
        new Translation2d(
            getVelocityXStationary(dist, targetHeight, maxBallYPos),
            getVelocityYStationary(maxBallYPos));

    Logger.recordOutput("AimUtil/shooterDistToHub", futurePos.getDistance(targetPos));
    ...
    return new ShooterMathResults(
        ...
        Radians.of(
                Constants.Shooter.AIMUTIL_HOOD_ANGLE_SCALAR.get()
                    * Math.atan2(shooterVelocityVec.getY(), shooterVelocityVec.getX()))
        ...
        MetersPerSecond.of(
                Constants.Shooter.AIMUTIL_SHOOTER_SPEED_SCALAR.get()
                    * Math.sqrt(
                        Math.pow(shooterVelocityVec.getX(), 2)
                            + Math.pow(shooterVelocityVec.getY(), 2)))
            .plus(MetersPerSecond.of(Constants.Shooter.AIMUTIL_SHOOTER_SPEED_ADDEND.get())),

        // The angle of the target vector is your robot heading
        // plus 90 because shooter is on right side of robot
        new Rotation2d(targetVec.getAngle().getMeasure()),
   ...
   */
  public static Rotation2d getShooterHeading() {
